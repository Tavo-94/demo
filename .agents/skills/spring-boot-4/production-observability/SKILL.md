...
- Agent leaves legacy MDC or `X-Correlation-ID` servlet filters active - delete them when enabling OTel, as Micrometer natively injects standard W3C `traceId` and `spanId` into SLF4J.
- Agent uses legacy Spring Boot 3 `management.otlp.tracing.endpoint` property - Spring Boot 4 completely changed this to `management.opentelemetry.tracing.export.otlp.endpoint`. The old property will be silently ignored, causing traces to be swallowed.
- Agent configures an HTTP endpoint but defaults to the gRPC exporter - If using `io.opentelemetry:opentelemetry-exporter-otlp`, it defaults to gRPC. Sending a gRPC stream to an HTTP port (like Tempo's 4318) fails silently. Use gRPC (4317) or strictly configure the HTTP transport.
- Agent points Spring Boot to a Tempo container but gets `404 page not found` logs from Micrometer - Spring Boot 4 tries to export metrics to `/v1/metrics` by default. Tempo is a pure trace backend and rejects metrics. Disable metrics with `management.otlp.metrics.export.enabled=false` if Prometheus is not available.
