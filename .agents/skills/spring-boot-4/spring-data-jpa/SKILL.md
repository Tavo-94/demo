...
- Agent adds `@Version` to an existing table without a migration - you MUST run `UPDATE table SET version = 0 WHERE version IS NULL` on existing rows, or `save()` will crash with Duplicate Key exceptions on older data.
